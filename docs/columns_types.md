...
| `ID_CLIENT` | INT64 | |
| `ID_LOCATION` | INT64 | |
| `SK_DATE_REFUND` | DATE | Partition key |
| `SK_DATE_REFUND_CREATED` | DATE | |
| `SK_DATE_REFUND_MODIFIED` | DATE | |
| `SK_SITE` | INT64 | |
| `SK_PRODUCT` | INT64 | |
| `SK_CUSTOMER` | INT64 | |
| `SK_LOCATION` | INT64 | |
...
| `AMOUNT` | NUMERIC | |
| `VAT_AMOUNT` | NUMERIC | |
| `REFUND_STATE_ID` | INT64 | |
| `REFUND_STATE` | STRING | Enum: PENDING, CANCELED, FINALIZED, PAYMENT_PROVIDER_ERROR, MADE_BY_HAND, UNKNOWN |
| `REFUND_TYPE_ID` | INT64 | |
| `REFUND_TYPE` | STRING | Enum: CLAIM, UNDER_DELIVERY, PAID_ORDER_ITEM_CANCELED, UNKNOWN |
## DIM_OFFER
| Kolumna | Typ w BigQuery | Uwagi |
| :--- | :--- | :--- |
| `SK_OFFER` | INT64 | |
