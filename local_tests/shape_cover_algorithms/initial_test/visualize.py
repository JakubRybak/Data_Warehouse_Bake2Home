"""
visualize.py - Benchmark summary plots.

Generates:
  plots/grid_heatmap.png      - heatmap shapes x algorithms
  plots/summary_bar.png       - overall ranking + time vs quality scatter
  plots/per_shape/<shape>.png - individual plot per shape
"""
import os
import sys
import csv
...
def plot_heatmap(shapes, algos, mat_n, shape_names):
    ...
    ax.set_title('Benchmark Results Heatmap: Shapes x Algorithms\n(color = circle count; white = DNF)',
                 fontsize=13, pad=12)
    plt.tight_layout()
    out = os.path.join(PLOTS_DIR, 'grid_heatmap.png')
...
def plot_summary(algos, mat_n, mat_t):
    ...
    axes[0].set_title('Ranking: Average Circle Count')
    ...
    plt.suptitle('Aggregate Benchmark Results for Shape Covering Algorithms',
                 fontsize=13, y=1.01)
    plt.tight_layout()
    out = os.path.join(PLOTS_DIR, 'summary_bar.png')
...
def plot_per_shape(shapes, algos, mat_n, mat_t, shape_names):
    ...
    for i, sid in enumerate(shapes):
        ...
        ax.set_title(f'{sid}: {shape_names.get(sid, "")} - Algorithm Comparison',
                     fontsize=11)
        ax.grid(True, axis='y', alpha=0.3)
        ax.set_ylim(bottom=0)

        plt.tight_layout()
        out = os.path.join(PER_SHAPE, f'{sid}.png')
        plt.savefig(out, dpi=150)
        plt.close()

    print(f"Saved {len(shapes)} per-shape plots to {PER_SHAPE}")
...
def plot_time_comparison(algos, mat_t, mat_n):
    ...
    # Use logarithmic scale if diff is huge and there are non-zero times
    ...
    ax.set_title('Time Comparison: Fastest vs Slowest Algorithms\nColor = Quality (Green is fewer circles)', fontsize=13)
    ...
    plt.tight_layout()
    out = os.path.join(PLOTS_DIR, 'time_comparison.png')
    plt.savefig(out, dpi=150)
    plt.close()
    print(f"Saved {out}")
